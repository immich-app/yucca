...
/** One connection's usage rolled up from its repositories' latest RGW meter readings. */
...
export class ConnectionMetricsRepository {
  ...
  /**
   * Aggregate each connection's repositories from `repositoryMeter`. Every connection is returned
   * (LEFT JOINs) so a connection with no metered repositories rolls up to zero. `bigint` SUM/COUNT
   * come back as strings from pg, so they're cast to `number` here.
   */
