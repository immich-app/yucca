...
Ceph gateway logs (the query_rgw_logs tool): radosgw's own access log, restricted to the buckets this account owns (one bucket per repository, named by repository id). Every request michael makes to storage on the user's behalf appears here a layer below michael's own log, already parsed into op (get_obj/put_obj/delete_obj/list_bucket/stat_bucket/…), bucket, status (Ceph's internal code, 0 = ok), http_status, latency (seconds) and request_id.
Use it to separate "michael failed" from "Ceph failed": michael reporting a 500 with matching RGW http_status:>=500 or high latency is a storage-side fault, whereas michael failing while RGW shows no request at all (or a clean 200) puts the fault in michael or the network between them. Two caveats: only the prod fsn1 gateways ship these logs, so an empty result can mean the account lives elsewhere rather than that nothing happened; and michael's own health-probe traffic never appears, because it uses a bucket outside this account.
