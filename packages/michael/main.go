...
// buildClusters builds one Storage per fronted cluster (keyed by code), plus
// the load-balancing pools concretely (for metrics registration + reconcile
// loops). Single-cluster deployments yield one entry under cfg.S3DefaultCluster.
...
// buildStorage returns one cluster's Storage: no backend source = single S3
// client (legacy, pool nil); source=file/dns = load-balancing Pool (returned
// concretely for metrics + reconcile).
