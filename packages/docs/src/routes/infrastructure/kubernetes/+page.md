---
title: Kubernetes and Flux
description: The Flux GitOps trees for the real clusters and the dev mirror, how a deploy flows from staging to production, and how to bootstrap a fresh cluster
order: 3
---

The `kubernetes/` directory holds **two parallel trees** with different consumers:
...
- **`apps/dev/local/` + `clusters/dev/local/`** is the dev-mirror tree the
  [Tiltfile](https://github.com/immich-app/yucca/blob/main/Tiltfile) consumes for local k3d dev. Tilt scans only
  ...
  can't resolve Flux variable substitutions). The local workflow itself is
  described in [k3d and Tilt](/development/kubernetes).

## GitOps deploy (staging → production)

The release process built on top of this pipeline, from the release-please PR to the production promotion and rollback, is described in [Releases and deployment](/development/releases).
...
2. **staging (automatic, in-cluster)** — the flux-operator `ResourceSetInputProvider`
   ...
   HelmRelease as the `YUCCA_IMAGE_TAG` substitution. No GHA job, no git commit.
3. **production (gated by the release PR)** — release-please stamps the next release
   ...
   the `YUCCA_IMAGE_TAG` substitution selects the matching `v<version>` images, published by the
...
The [Tiltfile](https://github.com/immich-app/yucca/blob/main/Tiltfile) derives **everything it deploys** from the
...
- `mock-oidc` and `rook-ceph` are **dev-only**. Prod swaps in a real IdP, and
  ...
  in [`ansible/ceph`](/infrastructure/ceph) / [`tf/`](/infrastructure/terraform)) — not this Rook cluster.
- The Rook-Ceph dev cluster is single-node/single-replica and synthesizes a
  loopback block device (k3d has no spare disk). See
  [`charts/platform/rook-ceph-cluster`](https://github.com/immich-app/yucca/tree/main/charts/platform/rook-ceph-cluster). `michael`'s S3
  credentials come from a full RGW user
  ([`charts/platform/ceph-objectuser`](https://github.com/immich-app/yucca/tree/main/charts/platform/ceph-objectuser)) whose Secret Rook
...
### 3. Seed the sources and entrypoint from the local checkout
...
> Rook dev cluster are dev-only. See the Dev vs prod section above.
