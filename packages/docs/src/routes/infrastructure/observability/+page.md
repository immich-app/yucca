---
title: Observability
description: The Grafana dashboards and alert rules yucca ships to the o11y cluster as one signed OCI bundle, and how they are consumed and edited
order: 8
---

The [o11y](https://github.com/immich-app/yucca/tree/main/o11y) directory holds
...
[`scripts/import-upstream.py`](https://github.com/immich-app/yucca/blob/main/o11y/scripts/import-upstream.py) (run it to refresh; it overwrites local edits) and
...
`folderRef: yucca` (the bundle's own `GrafanaFolder`), and
`datasourceUid: VictoriaMetricsFleet` on every query node - alert rules cannot use a
...
- **Grafana threshold semantics**: the condition is "query A > 0", and Grafana
  ...
  while firing (e.g. cert expiry alerts on seconds _inside_ the warning
...
- **Guarded absence**: absence-style rules use
  ...
  data that _disappears_ fires within minutes and self-resolves after 24h.
- **Scope**: only what nothing else delivers. Grafana-managed alerting is the
  ...
  prometheus/alertmanager on the [ceph cluster](/infrastructure/ceph), not from o11y.
...
CI ([`.github/workflows/o11y.yml`](https://github.com/immich-app/yucca/blob/main/.github/workflows/o11y.yml)) validates every dashboard, renders each into a
...
The generated CRs carry only sane defaults (`instanceSelector: {dashboards: grafana}`,
`folderRef: yucca`, `resyncPeriod: 10m`); o11y overlays cluster
specifics via its Kustomization. The `o11y/` sources stay the source of truth;
the bundle is built by
[`.github/scripts/render-o11y-manifests.sh`](https://github.com/immich-app/yucca/blob/main/.github/scripts/render-o11y-manifests.sh) at push time
and never committed.
...
- **Ref choice.** `tag: main` tracks every merge (fast); `semver`/`tag: latest`
  track releases only. Reference the tag _only_ (no `digest`) or auto-updates stop.
...
rule exclusions live in [`dashboards/.lint`](https://github.com/immich-app/yucca/blob/main/o11y/dashboards/.lint)). Alerts are plain
