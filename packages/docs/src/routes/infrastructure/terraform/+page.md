---
title: Terraform
description: The Terragrunt layout under tf, the partition and region model, 1Password secret injection, the discovery output contract and the ceph-cluster, talos-baremetal and netbird-env modules
order: 2
---
...
## The partition and region model
...
The talos stack is documented on the [Talos](/infrastructure/talos) page and in
the [cluster bring-up runbook](https://github.com/immich-app/yucca/blob/main/ansible/talos/docs/runbooks/cluster-bring-up.md)
(the TF + Ansible flow is interleaved — TF renders the inventory Ansible
consumes, then bootstraps the VMs Ansible created).
...
### Per-region metadata and role in region.hcl
...
The same pattern is used in [immich-app/devtools](https://github.com/immich-app/devtools) and is the Futo-wide
...
[infra.yml](https://github.com/immich-app/yucca/blob/main/.github/workflows/infra.yml) runs the staging stacks from GitHub Actions:
...
JWT-keypair item). Note: a copy of the _dev_ SA token won't work — it can't read
...
topology that [yuctl](/operations/yuctl) consumes — it reads the state object straight from S3 and
...
the _reference_ (`op://<vault>/<title>/password`).
...
3. **(Not yet TF-managed)** `onepassword_item` resources for cluster
   ...
   by Ansible at play time. See the [Ceph secrets document](https://github.com/immich-app/yucca/blob/main/ansible/ceph/docs/secrets.md) for the
   re-enable plan.
...
- **Ingress firewall** (`firewall.tf`): default-deny + per-service allow-lists
  ...
  Tailscale ranges (`trust_tailscale`). **Warning:** the host running `tf apply` must have
...
Run it (see "Running TF" above — needs 1Password unlocked + an on-LAN apply host):
...
peers are unambiguously _that region's_ mgmt nodes. A region layer can still
...
### The yucca to yucca-tags access model
...
destinations are _all_ of that layer's flagged groups — so flagging a group grants
...
_initiate_ — this policy never makes a tagged group a source. The union of these
...
### Declarative input in netbird.auto.tfvars
...
policy is _not_ declared here: the module generates it from every group flagged
...
CI ([infra.yml](https://github.com/immich-app/yucca/blob/main/.github/workflows/infra.yml)) applies `staging/global/netbird` in the staging
...
replaced the Tailscale subnet-router path). The [netbird-connect](https://github.com/immich-app/yucca/tree/main/.github/actions/netbird-connect)
...
Rotation affects all of them — see the [service-account token rotation runbook](https://github.com/immich-app/yucca/blob/main/ansible/ceph/docs/runbooks/rotate-sa-token.md)
...
7. See [adding a cluster](https://github.com/immich-app/yucca/blob/main/ansible/ceph/docs/adding-a-cluster.md)
   for the full walk-through.

## Related

- TF-first + op inject model: the [Ceph secrets document](https://github.com/immich-app/yucca/blob/main/ansible/ceph/docs/secrets.md)
- Immich devtools (upstream pattern): [immich-app/devtools/tf](https://github.com/immich-app/devtools/tree/main/tf)
