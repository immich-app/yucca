...
describe('Schedule', () => {
  ...
  it('filters unlinked repository IDs from ordering on update', async () => {
    const repositoryService = ctx.module.get(RepositoryService);
    const scheduleService = ctx.module.get(ScheduleService);

    const { repository: linked } = await repositoryService.createRepository(
      { name: 'Linked Repo', worm: false },
      ctx.backendId,
    );

    const { repository: unlinked } = await repositoryService.createRepository(
      { name: 'Unlinked Repo', worm: false },
      ctx.backendId,
    );
    ...
    const { schedule } = await scheduleService.createSchedule({
      ...
      cron: '0 0 1 1 *',
      repositories: [linked.id],
    });

    const { schedule: updated } = await scheduleService.updateSchedule(schedule.id, {
      repositories: [unlinked.id, linked.id],
    });

    expect(updated.repositories).toEqual([linked.id]);
  });
