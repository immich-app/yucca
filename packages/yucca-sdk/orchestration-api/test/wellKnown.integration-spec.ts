...
describe(YuccaWellKnown.name, () => {
  ...
  it('follows the pointer to /meta and caches the result', async () => {
    await expect(sut.getBaseUrl()).resolves.toBe('https://backups.futo.cloud/api');
  ...
  it('resolves legacy backend UUIDs and rejects unknown ones', async () => {
    fetchMock.mockImplementation((url) => {
      if (String(url) === 'https://futo.cloud/.well-known/yucca.json') {
        return Promise.resolve(
          jsonResponse({
            defaultBackend: 'father',
            backends: { father: { displayName: 'Father', api: 'https://legacy.example.test/api' } },
          }),
        );
      }
      return Promise.reject(new Error(`Unexpected fetch: ${String(url)}`));
    ...
    await expect(sut.getBaseUrlById('father')).resolves.toBe('https://legacy.example.test/api');
    await expect(sut.getBaseUrlById('missing')).rejects.toThrow(/reauthentication/);
  });
});
