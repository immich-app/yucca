import { z } from 'zod';
import { YUCCA_LEGACY_WELL_KNOWN, YUCCA_WELL_KNOWN } from './const';
import { evaluateConnectionsMath } from './utils/connectionsMath';

// Per-environment discovery: the well-known URL serves a pointer
// ({"meta_url": ...}) to that environment's public GET /api/meta document.
...
const legacyWellKnownSchema = z
  .object({
    backends: z.record(z.string(), z.object({ displayName: z.string(), api: z.url() })),
...
export class YuccaWellKnown {
  ...
  private failedAt = 0;
  private legacyData?: z.infer<typeof legacyWellKnownSchema>;
  ...
  async get(): Promise<Meta> {
    ...
    try {
      ...
      // Commit cache state only after both documents have fully validated.
      this.data = next;
  ...
  async getBaseUrl(): Promise<string> {
    ...
    return meta.api_root;
  }

  // Compatibility for backend rows persisted against the old multi-backend
  // document. Unknown IDs fail closed instead of routing an access token to the
  // new default environment.
  async getBaseUrlById(id: string): Promise<string> {
    this.legacyData ??= await fetchValidated(YUCCA_LEGACY_WELL_KNOWN, legacyWellKnownSchema);
    const backend = this.legacyData.backends[id];
    if (!backend) {
      throw new Error(`Legacy Yucca backend '${id}' is no longer advertised; reauthentication is required`);
    }
    return backend.api;
  }
  ...
  // Effective configuration precedence: global < site < storage cluster.
