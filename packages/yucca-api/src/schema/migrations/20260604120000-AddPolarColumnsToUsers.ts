...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  await sql`ALTER TABLE "users" ADD CONSTRAINT "users_polarUserId_uq" UNIQUE ("polarUserId"); -- missing in target`.execute(
    db,
  );
  await sql`ALTER TABLE "users" ADD CONSTRAINT "users_polarSubscriptionId_uq" UNIQUE ("polarSubscriptionId"); -- missing in target`.execute(
    db,
  );
  await sql`CREATE INDEX "users_polarUserId_idx" ON "users" ("polarUserId"); -- missing in target`.execute(db);
  await sql`CREATE INDEX "users_polarSubscriptionId_idx" ON "users" ("polarSubscriptionId"); -- missing in target`.execute(
    db,
  );
}
