# Root Terragrunt config, merged into every stack via include "root".
# Derives partition/region/stack (+ slug) from the path, merges region.hcl
# metadata, and configures the S3 state backend (yucca-tf-state at OVH Paris,
# key yucca/<partition>/<region>/<stack>/). Providers are per-stack (versions.tf).

locals {
  # <partition>/<region>/<stack...> parsed from the path; `stack` = everything
  # after the region joined (regions may nest sub-stacks with their own state
  # key). Canonical slug = <partition>-<region>.
  ...
  # Region metadata from the nearest region.hcl; "" fallback → fields null.
