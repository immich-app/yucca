# Per-cluster password items; titles from the ceph-cluster module's `secrets`
# output. 1P auth via OP_SERVICE_ACCOUNT_TOKEN (CI: OP_TF_YUCCA_PROD_ENV_WRITE;
...
locals {
  # Out-of-band roles (see header). alertmanager_webhook is an external Zulip
  # URL, not a generated credential — managing it would overwrite the live URL
  # with a random password and silently break alert delivery.
  ...
  # (cluster, role) -> { vault, title, length }, dropping out-of-band roles.
