...
# Identity registry, used for server operator access: `server`-mapped group
# members' SSH keys land in the ops account (group_vars/all/operators.yml below).
module "identity" {
  ...
  value = {
    for k, m in module.cluster : k => {
      ...
      files = merge(m.rendered_files, {
        # Operator public keys for the shared ops account; gitignored.
