# Login users come from shared/modules/identity — manage people/groups there, not here.
...
module "core" {
  ...
  # Spine IRB 10.40.11.1 routes kube↔kube-cp.
  kube_cp = {
  ...
  # Multi-home: add an entry with prepend>0 + lower local_pref
  # (core-fabric/transit.tf; those knobs need a provider regen).
  local_as = 402421
  transits = {
    core-backbone = {
      interface = "et-0/0/27"
