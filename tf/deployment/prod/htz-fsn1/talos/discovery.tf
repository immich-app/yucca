...
locals {
  # REGION-scoped titles (partition alone collides across regions). Single
  # source for secrets.tf (writes the 1P items) and the discovery refs below.
...
output "discovery" {
  ...
  value = {
    ...
    kubernetes = {
      ...
      # Node NAME → IP maps, consumed by the netbird stack (DNS records).
