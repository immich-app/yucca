...
# Fabric creds for kubernetes/apps/prod/htz-fsn1/netops/ — never in git: the
# `netops` Junos login SSH key/password (fabric stack netops_users) + Grafana
# admin. Hand-created versions died with the cluster on the 2026-07 rebuild;
# TF now provisions from 1P so rebuilds restore them. TF owns namespace +
# Secrets (namespace carries the VM hostPath PVC and must survive flux prunes);
# Flux owns the workloads.
...
# junos-exporter + oxidized mount key `id_ed25519`, log in as `netops`.
