...
# One cluster, driven over apid (controlplane.tf / workers.tf). Planes:
#   etcd + apiserver + API VIP → kube-cp VLAN 11 (10.40.11.0/24)
#   worker↔worker pod east-west → kube VLAN 10 (50G, Cilium BGP)
#   CP↔worker → routed kube↔kube-cp via the spine IRBs
#   operators/CI → NetBird (backup plane only; fabric routes pinned in configs)
...
locals {
  ...
  # Fabric bonds/VLANs + Cilium datapath MTU. Switch L2 9216, IRBs 9202. Public
  # NICs/wt0 keep their MTUs; Cilium auto-detect off (once clamped to wt0's 1280).
  fabric_mtu = 9000

  # Factory installer keeps the schematic's extensions; consulted on install/upgrade.
  install_image = "factory.talos.dev/metal-installer/${local.talos_schematic_id}:v${local.c.talos_version}"

  # API endpoint: NetBird DNS name → Talos-elected VIP 10.40.11.5 (kube-cp; same
  # IP as the retired hcloud LB, so the record carried over). In cert SANs + node
  # host entries; mesh peers resolve it via the yucca.futo.network zone (netbird
  # stack) over the kube-cp route. Kubelets dial KubePrism 127.0.0.1:7445 instead.
  ...
  # Operator/backup plane. Distinct setup keys: CPs → [talos, talos_cp], workers →
  # [talos]. talos_cp = CP-only kube-cp router group — a worker in it is treated as
  # a router and never installs the client route. Wiring: netbird stack.
  ...
  # nodeIP: CPs → kube-cp VLAN 11, workers → kube VLAN 10. clusterDNS must be
  # explicit: Talos defaults 10.96.0.10 and does NOT derive it from serviceSubnets.
  ...
  # extraMounts: kubelet only sees an allow-listed /var set; /var/mnt hostPath used
  # by the netops VictoriaMetrics 30d buffer (kubernetes/apps/prod/htz-fsn1/netops/).
  ...
  # nvme1n1 (non-install NVMe, 1.92TB) split 50/50: u-localpv xfs → OpenEBS LocalPV
  # (CNPG node-local PG; replication is Postgres-level, deliberately not block);
  # r-mayastor raw → Mayastor DiskPool (repl=2; VictoriaMetrics). System disk is
  ...
  # Mayastor prereqs: 2GiB of 2MiB hugepages (SPDK), nvme_tcp (NVMe-oF attach),
  # engine label. Kubelet only discovers hugepages at startup — restart it once.
  ...
  # Install patch is per-node (disk serial), appended in controlplane.tf/workers.tf.
...
locals {
  # All provider dials (bootstrap/kubeconfig/health, helm/kubernetes) use the
  # kube-cp IPs over the NetBird kube-cp route (in cert SANs). Greenfield: the
  # route appears once the first CP boots and joins the mesh.
