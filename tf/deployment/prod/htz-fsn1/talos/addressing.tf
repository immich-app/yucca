# Same module the fabric + netbird stacks read; nothing hardcoded.
...
# Only for the leaf vme address (netops-secrets.tf hyperglass).
module "addr_cls1" {
  source     = "../../../../shared/modules/fabric-addressing"
  site_id    = var.site_id
  cluster_id = 1
...
locals {
  ...
  # spice RGW frontend, routed by the spine (fabric stack public_routing);
  # michael's S3 path.
