# Prod cluster topology — source of truth is clusters.auto.tfvars. One object,
# not a map: the bring-up is bespoke.
...
variable "cluster" {
  ...
  type = object({
    ...
    # Schematic id / image URLs derive from schematic.yaml (image.tf) — not inputs.
    ...
    # cp_ip = post-install VLAN-11 address (etcd/apiserver/nodeIP); maint_ip =
    # Hetzner public IP in maintenance mode, endpoint for the one-time install apply.
    cps = list(object({
    ...
    # API VIP = cidrhost(kube_cp, vip_offset), etcd-elected. 5 keeps the retired
    # hcloud LB's IP so the api_dns_name record carried over.
    vip_offset = number

    # ── Bare-metal workers (Hetzner Robot; kube fabric VLAN 10) ───────────────
    # Same maint_ip/fabric_ip semantics; nodeIP = fabric_ip.
    workers = list(object({
      name           = string
      install_serial = string
    ...
    # Worker default route: kube IRB gateway (fabric transit) when true, else
    # the Hetzner public NIC (DHCP).
...
# Extra source CIDRs on the host firewall (apid 50000 + apiserver 6443), beyond
# node planes + NetBird. The `tf apply` host MUST be in one or apid hangs.
