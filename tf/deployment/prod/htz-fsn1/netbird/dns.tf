# NetBird internal DNS — yucca.futo.network zone + A records. Reverse-hostname
# scheme: <thing>.<cluster>.<region>.<provider>.yucca.futo.network. Zone is
# account-wide but lives here (only site using it); move to a global netbird
# stack if a second site needs it.
...
locals {
  # Node names + IPs from the talos stack's discovery output (talos-discovery.tf).
...
# API endpoint — Talos-elected VIP on kube-cp (etcd parks it on a healthy CP);
