# Stretched VLANs — L2-only on the spine except kube (IRB with node_bgp;
# bgp-nodes.tf) and kube-cp (IRB with kube_cp). Other gateways live on the leaves.
locals {
  spine_vlans = merge({
...
# kube-cp IRB — spine is the .1 gateway; worker↔CP routes irb.<kube>↔irb.<kube-cp>.
...
# so the spine routes kube↔cls-public (workers' RGW path); public hosts route
# the kube net back via this address.
