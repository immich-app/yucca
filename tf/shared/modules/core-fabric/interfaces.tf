locals {
  ...
  # Physical mtu only; unit 0 left empty.
...
# Each ae = the two 25G ports cabled to one node (one per VC member).
# ae device-count is provider-managed.
locals {
...
# kube-cp is the CPs' only fabric presence.
locals {
...
# Physical mtu comes from the access-port set above.
