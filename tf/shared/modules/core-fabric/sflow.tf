# ── sFlow export ─────────────────────────────────────────────────────────────
# High-resolution bandwidth feed → sflow-rt (father, internal LB VIP via Cilium
# iBGP /32); junos_exporter stays for device health. Raw set-config: `protocols
# sflow` has no typed jeremmfr resource.
