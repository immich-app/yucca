...
# Auto host names from the shared node-names inventory (backs talos + ceph);
# cluster_name seeds the shuffle, explicit names are excluded from the pool.
module "names" {
  source       = "../node-names"
...
# Preserve shuffle state across the node-names refactor (names must not re-randomize).
moved {
