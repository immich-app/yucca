#!/usr/bin/env bash
# A SINGLE `op run` (no extra probe → no extra auth prompts), appending an
# "unlock 1Password" hint on auth failure (locked 1P otherwise dies cryptically
# downstream). Usage from the repo root: tf/op-run.sh <cmd> [args...];
