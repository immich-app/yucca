...
| Renders `inventory-provision.ini` when `provision_profile` set           | Create one `host_vars/<hostname>.yml` per node (hardware topology) |
...
clusters = {
  ...
  mesa = {
    domain            = "prod.htz-fsn1.htz.futo.cloud"
    partition         = "prod"
    region            = "htz-fsn1"
    provider_code     = "htz"
    ...
    vault             = "yucca_tf_prod"      # the cluster's partition vault
...
cp inventories/staging-austin/sietch/group_vars/all/vars.yml \
   inventories/prod-htz-fsn1/mesa/group_vars/all/vars.yml
```
...
cluster_name: mesa
cluster_role: ceph
cluster_domain: prod.htz-fsn1.htz.futo.cloud
...
```bash
export CEPH_ENV=inventories/prod-htz-fsn1/mesa/inventory.ini
mise run preflight
```
...
| `inventories/<partition>-<region>/<cluster>/group_vars/all/vars.yml` | `inventories/*/inventory-provision.ini`    |
| `inventories/<partition>-<region>/<cluster>/host_vars/<hostname>.yml` | `inventories/*/inventory-destroy.ini`      |
| `inventories/<partition>-<region>/<cluster>/installimage/*.tpl`       | `inventories/*/secrets.yml.tpl`            |
